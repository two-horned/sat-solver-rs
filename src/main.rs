...
fn parse_numbers(line: &str) -> Result<Vec<isize>, ParseProblemError> {
  line
    ...
    .map(|x| {
      str::parse::<isize>(x)
        .map_err(|_| ParseProblemError::NotANumber(x.to_string()))
    })
    .collect()
}
...
fn parse_header(line: String) -> Result<Header, ParseProblemError> {
  ...
  Ok(Header { vrs: nums[0] as usize, cls: nums[1] as usize })
}
...
fn main() -> io::Result<()> {
  ...
  if let Some(header) = h {
    ...
    let a = {
      let layout = unsafe {
        Layout::from_size_align_unchecked(BLOCK_SIZE as usize * len, 32)
      };
